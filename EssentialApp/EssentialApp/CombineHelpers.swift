...
//  EssentialApp
//
//  Created by Jessie Elliott on 7/28/24.
//
...
extension DispatchQueue {
    ...
    struct ImmediateWhenOnMainQueueScheduler: Scheduler {
        ...
        func schedule(options: SchedulerOptions?, _ action: @escaping () -> Void) {
            ...
            // The main thread is not guaranteed to be running the main queue (it could be a background queue)
            // If we can guarantee that we're on the main queue, then we automatically guarantee we're on the main thread
...
struct AnyScheduler<SchedulerTimeType: Strideable, SchedulerOptions>: Scheduler where SchedulerTimeType.Stride: SchedulerTimeIntervalConvertible {
    ...
    init<S>(_ scheduler: S) where SchedulerTimeType == S.SchedulerTimeType, SchedulerOptions ==
        S.SchedulerOptions, S: Scheduler {
            _now = { scheduler.now }
